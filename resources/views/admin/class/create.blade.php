...
@section('content')
    ...
    <section class="overflow-hidden rounded-[2rem] border border-slate-200 bg-white p-8 shadow-xl shadow-slate-200/70">
        <form method="POST" action="{{ route('admin.manage-class.store') }}" class="space-y-6">
            @csrf
            ...
            <div class="flex flex-wrap items-center justify-end gap-3">
                <a href="{{ route('admin.manage-class') }}" class="rounded-2xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-100">Cancel</a>
