...
@section('header-title')
    <p class="text-sm font-semibold uppercase tracking-[0.22em] text-sky-700">Add Class</p>
@endsection

@section('content')
    <section class="space-y-2">
        <h2 class="text-3xl font-semibold tracking-tight text-slate-950">Create a new class</h2>
        <p class="text-sm text-slate-600">Set the basic class data before adding students and teachers.</p>
    </section>
        ...
        <form method="POST" action="{{ route('admin.class.store') }}" class="space-y-6">
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                <label class="block">
                    ...
                    <input type="text" name="class_name" placeholder="Class A - Pagi" class="w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-sky-500 focus:bg-white" />
                    @error('class_name')
