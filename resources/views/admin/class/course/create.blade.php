...
@section('header-title')
    <p class="text-sm font-semibold uppercase tracking-[0.22em] text-sky-700">Tambah Mata Kuliah</p>
@endsection

@section('content')
    <section class="space-y-2">
        <p class="text-sm font-semibold uppercase tracking-[0.22em] text-slate-500">{{ $class->class_code }}</p>
        <h2 class="text-3xl font-semibold tracking-tight text-slate-950">Buat mata kuliah untuk {{ $class->class_name }}</h2>
        <p class="text-sm text-slate-600">Tentukan nama mata kuliah dan pilih dosen untuk kelas ini.</p>
    </section>
    ...
    <section class="overflow-hidden rounded-[2rem] border border-slate-200 bg-white p-8 shadow-xl shadow-slate-200/70">
        <form method="POST" action="{{ route('admin.manage-class.course.store', $class->id) }}" class="space-y-6">
            ...
            <div class="flex flex-wrap items-center justify-end gap-3">
                ...
                <button type="submit" class="rounded-2xl bg-slate-900 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-slate-900/20 transition hover:-translate-y-0.5 hover:bg-slate-800">Simpan Mata Kuliah</button>
            </div>
        </form>
    </section>
@endsection
