...
@section('header-title')
    <p class="text-sm font-semibold uppercase tracking-[0.22em] text-sky-700">Create Course</p>
@endsection
...
@section('content')
    <section class="space-y-2">
        ...
        <p class="text-sm text-slate-600">Set the course name and choose the teacher for this class.</p>
    </section>
        ...
        <form method="POST" action="{{ route('admin.manage-class.course.store', $class->id) }}" class="space-y-6">
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                <label class="block">
                    ...
                    <input type="text" name="course_name" value="{{ old('course_name') }}" placeholder="Introduction to Programming" class="w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-sky-500 focus:bg-white" />
                    @error('course_name')
