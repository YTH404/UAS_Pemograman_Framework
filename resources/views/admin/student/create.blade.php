...
@section('header-title')
    <p class="text-sm font-semibold uppercase tracking-[0.22em] text-sky-700">Tambah Mahasiswa</p>
@endsection

@section('content')
    <section class="space-y-2">
        <h2 class="text-3xl font-semibold tracking-tight text-slate-950">Buat akun mahasiswa baru</h2>
        <p class="text-sm text-slate-600">Tambahkan detail Login dan pilih kelas yang diikuti mahasiswa.</p>
    </section>
        ...
        <form method="POST" action="{{ route('admin.student.store') }}" class="space-y-6">
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                <label class="block">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="Nama mahasiswa" class="w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-sky-500 focus:bg-white" />
                    @error('name')
                ...
                <label class="block lg:col-span-2">
                    <span class="mb-2 block text-sm font-medium text-slate-700">Kelas</span>
            ...
            <div class="flex flex-wrap items-center justify-end gap-3">
                <a href="{{ route('admin.student.index') }}" class="rounded-2xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-100">Batal</a>
                <button type="submit" class="rounded-2xl bg-slate-900 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-slate-900/20 transition hover:-translate-y-0.5 hover:bg-slate-800">Simpan Mahasiswa</button>
            </div>
        </form>
    </section>
@endsection
