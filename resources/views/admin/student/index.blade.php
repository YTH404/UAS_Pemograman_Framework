...
@section('content')
    <section class="space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h2 class="mt-2 text-3xl font-semibold tracking-tight text-slate-950">Manage students and class placement</h2>
            </div>
                ...
                <tbody class="divide-y divide-slate-200">
                    @forelse ($students as $student)
                        <tr class="transition-colors hover:bg-slate-50">
                            ...
                            <td class="px-6 py-4 text-sm text-slate-600">{{ $student->studentClass?->class?->class_name ?? 'No class yet' }}</td>
